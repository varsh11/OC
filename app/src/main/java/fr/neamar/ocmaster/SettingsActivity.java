...
import fr.neamar.ocmaster.dataprovider.SearchProvider;
...
public class SettingsActivity extends PreferenceActivity implements
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            ...
            multiPreference.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                ...
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    ...
                    OCApplication.getDataHandler(SettingsActivity.this).getAppProvider().reload();
                    return false;
                }
            });
            if (!hasExcludedApps(prefs)) {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if (key.equalsIgnoreCase("icons-pack")) {
            OCApplication.getIconsHandler(this).loadIconsPack(sharedPreferences.getString(key, "default"));
        }

        if (key.equalsIgnoreCase("sort-apps")) {
            // Reload application list
            final AppProvider provider = OCApplication.getDataHandler(this).getAppProvider();
            if (provider != null) {
                provider.reload();
            }
        }
    ...
    private void fixSummaries() {
        int historyLength = OCApplication.getDataHandler(this).getHistoryLength();
        if (historyLength > 5) {
    ...
    protected void setListPreferenceIconsPacksData(ListPreference lp) {
        IconsHandler iph = OCApplication.getIconsHandler(this);
